...
/**
 * This is meant as an integration test, as such is ignored as part of the unit tests.
 */
...
public class IntegrationTests {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.print("Enter Your Google Voice Username, eg user@gmail.com:\t");
		...
		try {
			...
			switch (testNr) {
				case 1: // 1: Multi phone enable / disable
				{
					...
					for (Phone phone : phones) {
						System.out.println(phone.getName() + " " + phone.getId() + " " + !voice.getSettings(false).isPhoneDisabled(phone.getId()));
					}
				...
				{
					System.out.println("******** Starting Test " + testNr + " ********");
					for (Greeting type : voice.getVoicemailList(true)) {
				...
				case 9: // 9: Read all settings - pure json driven
				{
					...
					JsonObject origSettings = gson.fromJson(jsonData, JsonObject.class);
					System.out.println(origSettings == null ? "null" : origSettings.toString());
...
//								System.out.println(objFromAllSettings.toString(4));
					...
					for (Group group : groups) {
						...
						if (group.getId().equals(groupId)) {
							System.out.println("Changing settings for Group: " + group.getName());
							group.setCustomDirectConnect(true);
							group.setDirectConnect(true);
							group.setCustomForwarding(true);
							List<DisabledForwardingId> disList = new ArrayList<>();
							disList.add(new DisabledForwardingId("1", true));
							group.setDisabledForwardingIds(disList);
							group.setCustomGreeting(true);
							group.setGreetingId(2);
							voice.setNewGroupSettings(group);
						}
					}
...
//								System.out.println(settings3.toJsonObject().toString(4));
				...
				case 13: // 13: List Default Phones and Enabled/Disabled Setting
				{
					...
					for (Integer phoneint : phoneList) {
						inner:
						for (int ig = 0; ig < actualPhoneArray.length; ig++) {
